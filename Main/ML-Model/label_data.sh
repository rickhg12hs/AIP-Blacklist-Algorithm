...
concatenated_data_labeled=$output_folder/ML_Model_Data/concatenated_data_labeled.csv
...
# and then append this data to the historical_concatenated_data
...
for line in $(cat $previous_24_hour_data);
    do
      if [[ $line =~ "SrcAddr" ]]; then
        ...
        IP=$(echo $line|awk -F, '{print $1}')
        if grep "$IP" $new_24_hour_data; then
          line="${line},1" > /dev/null
          echo $line >> $previous_24_hour_data_labeled
        else
          line="${line},0" > /dev/null
          echo $line >> $previous_24_hour_data_labeled
        fi
      fi
    done
